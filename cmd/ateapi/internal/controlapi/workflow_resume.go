...
// workerHoldingStaleClaim returns the worker already holding an assignment for
// the actor, or nil if none does.
//
// A claim writes the assignment to the worker before it writes the actor, so
// ateapi dying in between records a placement the actor knows nothing about.
// Picking a second worker would then leave the first one holding capacity for
// an actor it will never host, and nothing else reclaims it.
//
// A worker whose pool no longer admits the actor (its worker_selector changed
// after the failed attempt) is released here instead, leaving this resume to
// schedule afresh. That release has to finish before the actor can be placed
// anywhere else: an actor is hosted by at most one worker, so a second
// placement while the first still holds it is refused.
func (w *ActorWorkflow) workerHoldingStaleClaim(ctx context.Context, actor *ateapipb.Actor, constraints scheduling.Constraints) (*ateapipb.Worker, error) {
	...
	// Only Applies, not HasRoom: this worker is already hosting the actor, so
	// the actor's own resources are counted against the worker's capacity and a
	// room check would reject a placement that is already in force.
	if w.scheduler.Applies(worker, constraints) {
		return worker, nil
	}

	released, err := w.store.ReleaseActorFromWorker(ctx, workerName, worker.GetMetadata().GetVersion(), actorUID)
	if err != nil {
		return nil, fmt.Errorf("while releasing stale claim on worker %q: %w", workerName, err)
	}
	// Scheduling runs next and reads the cache, which still counts this actor
	// against the worker until the change event lands.
...
func (w *ActorWorkflow) assignWorkerAttempt(ctx context.Context, actorRef resources.ActorRef, actor *ateapipb.Actor, actorTemplate *ateapipb.ActorTemplate) (_ *ateapipb.Actor, _ *ateapipb.Worker, err error) {
	...
	// A claim writes the assignment before the actor, so ateapi dying in
	// between leaves a placement recorded against a worker that the actor
	// knows nothing about. Ask the store which worker, if any, holds one.
	...
	// The bind does not change the worker's identity, which is all the actor's
	// side of the assignment records.
