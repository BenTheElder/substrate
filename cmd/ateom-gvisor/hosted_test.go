...
func TestAdmitActorKeepsAHostedActorsSlot(t *testing.T) {
	...
	if _, _, err := s.admitActor(resources.ActorAttribution{UID: "actor-b"}); err == nil {
		t.Error("admitted a second actor past the ceiling")
	}
...
func TestGracefulShutdownWaitsForInFlightRPCs(t *testing.T) {
	...
	release := s.inFlight.Add("actor-a", rpcCheckpointWorkload, func() {}, false)
...
func TestGracefulShutdownCancelsEveryStartup(t *testing.T) {
	...
	for _, uid := range []string{"actor-a", "actor-b"} {
		defer s.inFlight.Add(uid, rpcRunWorkload, func() { canceled <- uid }, true)()
	}
...
func TestBeginRPCRegistersBeforeCheckingForADrain(t *testing.T) {
	...
	release, err := s.beginRPC("actor-a", rpcRunWorkload, func() {}, true)
	if err != nil {
	...
	if _, err := s.beginRPC("actor-b", rpcRunWorkload, func() {}, true); err == nil {
