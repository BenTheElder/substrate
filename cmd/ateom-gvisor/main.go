...
func (s *AteomService) gracefulShutdown(ctx context.Context) {
	...
	//
	// A wait on the in-flight count, not on a lock: activations do not serialize,
	// so holding any one actor's lock says nothing about the others.
	//
...
func (s *AteomService) RunWorkload(ctx context.Context, req *ateompb.RunWorkloadRequest) (resp *ateompb.RunWorkloadResponse, retErr error) {
	...
	// namespace.
	if err := setupBundleRootfs(act, req.GetActorUid(), ocispec.PauseContainer); err != nil {
