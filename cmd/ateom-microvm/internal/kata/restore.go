...
// ReconstructSharedDirFromRootfs rebuilds the virtio-fs shared dir for id from a
// golden OCI rootfs directory, instead of from a shipped shared-dir tar. With a
// read-only rootfs (Root.Readonly) the base never diverges from golden, so the
// checkpoint omits shared-dir.tar and restore re-derives the (immutable) shared
// dir from the re-extracted OCI bundle (content-addressed by image digest).
//
// The layout mirrors what the kata shim produced at run time:
// <SharedDir>/<id>/rootfs (container id == sandbox id in the single-container
// model), which find-paths re-opens by relative path. The copy is deterministic
// because rootfsDir is golden; a read-only bind of rootfsDir would avoid it but
// adds an unmount to the teardown path.
func ReconstructSharedDirFromRootfs(ctx context.Context, rootfsDir, id string) error {
	root := SharedDir(id)
	if err := os.RemoveAll(root); err != nil {
		return fmt.Errorf("clearing shared dir %q: %w", root, err)
	}
	dst := filepath.Join(root, id, "rootfs")
	if err := os.MkdirAll(dst, 0o755); err != nil {
		return fmt.Errorf("creating shared dir %q: %w", dst, err)
	}
	// cp -a preserves perms/owners/symlinks/timestamps. The trailing "/." copies
	// the directory CONTENTS into dst (not rootfsDir itself as a child).
	cmd := exec.CommandContext(ctx, "cp", "-a", rootfsDir+"/.", dst)
	var stderr strings.Builder
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("copying golden rootfs into shared dir: %w (%s)", err, strings.TrimSpace(stderr.String()))
	}
	return nil
}
