...
func ReconstructSharedDirFromImage(ctx context.Context, bundleRootfs, restoreID, sourceID string) error {
	...
	// M0 (large-image overlay): bind-mount the image rootfs into the find-paths
	// location instead of cp -a — O(1) setup vs O(image). The guest-visible path
	// (<sourceID>/rootfs) is unchanged, so the carrier bind + overlay lowerdir +
	// find-paths are untouched; cross-node consistency relies on the bundle being a
	// deterministic unpack of the same image. (A shared digest-keyed cache as the bind
	// source is the M2 step; here we bind the per-actor bundle to test find-paths over
	// a bind submount.)
	//
	// Drop any stale bind FIRST (lazy if busy) so we never recurse into — and delete —
	// the bind source; then ensure a clean mountpoint. We deliberately do NOT
	// RemoveAll(root) (it would chase a live bind into bundleRootfs).
