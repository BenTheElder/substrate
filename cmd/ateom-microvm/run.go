...
func ensureKataCompatibleSpec(bundle, id, netnsPath string) error {
	...
	// Mount the container rootfs READ-ONLY. The golden base is served over
	// virtio-fs and must never be written: writability is provided instead by the
	// tmpfs mounts in defaultKataMounts, which live in guest RAM (captured by the
	// CH memory snapshot). Because the container can't write to "/", the host
	// shared dir never diverges from golden — so the checkpoint ships no
	// shared-dir.tar and restore re-derives the (immutable) base from the
	// re-extracted OCI bundle by image digest. Tradeoff vs. the erofs/overlay
	// paths: the rootfs is writable only where a tmpfs is mounted, not everywhere;
	// in exchange this needs no kata fork, no erofs, and no host-kernel erofs
	// (so it runs on COS).
	if spec.Root == nil {
...
// produces (the proven-good shape for the kata agent), plus the writable tmpfs
// mounts that make the read-only rootfs usable.
//
// With Root.Readonly the container root is immutable, so every path a workload
// writes to must be backed by one of these tmpfs mounts (guest RAM, captured by
// the CH memory snapshot). A tmpfs mount SHADOWS any golden content at that path,
// so only mount tmpfs on dirs that should start empty (/tmp, /run, /var/tmp,
// scratch). Dynamic /etc files (resolv.conf, hostname) are delivered by kata as
// individual mounts, not by writing into the RO rootfs, so RO /etc is fine.
// Per-mount size= bounds guest RAM and snapshot growth — tune per workload.
func defaultKataMounts() []specs.Mount {
	return []specs.Mount{
		...
		// Writable scratch over the read-only rootfs (guest RAM).
		{Destination: "/tmp", Type: "tmpfs", Source: "tmpfs", Options: []string{"nosuid", "nodev", "mode=1777", "size=256m"}},
		{Destination: "/var", Type: "tmpfs", Source: "tmpfs", Options: []string{"nosuid", "nodev", "mode=755", "size=512m"}},
	}
}
...
func (s *AteomService) CheckpointWorkload(ctx context.Context, req *ateompb.CheckpointWorkloadRequest) (*ateompb.CheckpointWorkloadResponse, error) {
	...
	// No filesystem capture: the container rootfs is read-only (Root.Readonly),
	// served over virtio-fs from a golden base that the container never writes, so
	// the host shared dir never diverges from golden. All writes live in the
	// guest-RAM tmpfs mounts, which the CH memory snapshot below captures. The
	// snapshot is therefore memory-only ({config.json, state.json, memory-ranges});
	// restore re-derives the shared dir from the re-extracted OCI bundle (by image
	// digest), so no shared-dir.tar is shipped.
	...
	// (config.json + state.json + memory-ranges — memory-only, no shared-dir.tar),
	// not gVisor's fixed set.
...
// memory-ranges — memory-only) has been downloaded to RestoreStateDir, and the
// OCI bundle's rootfs/ has been re-extracted (golden, by image digest).
//
// Steps:
//  1. Reconstruct the virtio-fs shared dir (container rootfs) from the golden
//     OCI rootfs so find-paths can re-open the paths the snapshot references.
...
func (s *AteomService) RestoreWorkload(ctx context.Context, req *ateompb.RestoreWorkloadRequest) (resp *ateompb.RestoreWorkloadResponse, retErr error) {
	...
	// 1. Reconstruct the virtio-fs shared dir from the GOLDEN OCI rootfs (not a
	// shipped tar). The rootfs is read-only, so the base never diverged from
	// golden; atelet re-extracts <bundle>/rootfs on every Restore (content-
	// addressed by image digest), and we re-derive the (immutable) shared dir
	// locally. find-paths then re-opens the same relative paths the snapshot
	// references.
	containers := req.GetSpec().GetContainers()
	if len(containers) != 1 {
		return nil, status.Errorf(codes.Unimplemented, "ateom-microvm supports exactly one container, got %d", len(containers))
	}
	bundle := ateompath.OCIBundlePath(ns, name, id, containers[0].GetName())
	if err := kata.ReconstructSharedDirFromRootfs(ctx, filepath.Join(bundle, "rootfs"), id); err != nil {
		return nil, fmt.Errorf("while reconstructing shared dir from golden rootfs: %w", err)
	}
