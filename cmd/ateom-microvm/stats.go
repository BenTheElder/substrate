...
func (s *AteomService) GetActiveWorkloadStats(ctx context.Context, req *ateompb.GetActiveWorkloadStatsRequest) (*ateompb.GetActiveWorkloadStatsResponse, error) {
	...
	for i, h := range hosted {
		...
		go func() {
			defer wg.Done()
			samples[i] = pendingSample(&h.attribution)
			select {
			case slots <- struct{}{}:
				defer func() { <-slots }()
			case <-sweepCtx.Done():
			...
			sample, err := s.sampleGuest(sweepCtx, &h.attribution)
			if errors.Is(err, errStaleGuestTarget) {
				stale.Store(true)
			}
			// Otherwise an error is boot, restore, teardown in progress, or a
			// guest that has stopped answering: all routine, none of them an
			// error.
			if err == nil {
				samples[i] = sample
			}
