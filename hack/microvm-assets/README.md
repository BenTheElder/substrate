...
The `microvm` runtime (`cmd/ateom-cloud-hypervisor`, kata + cloud-hypervisor) fetches its
...
mounts it into the node and labels the node `ate.dev/runtime=microvm`.
...
2. **Bring up the cluster + control plane:**
   ```sh
   hack/create-kind-cluster.sh        # mounts /dev/kvm, labels node ate.dev/runtime=microvm
