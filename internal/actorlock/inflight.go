...
// Only startups are cancelable; checkpoints must finish saving state.
func (f *InFlight) Add(actorUID, name string, cancel context.CancelFunc, cancelable bool) func() {
	f.mu.Lock()
	f.rpcs[actorUID] = &rpc{name: name, cancel: cancel, cancelable: cancelable}
	f.notifyLocked()
	f.mu.Unlock()
