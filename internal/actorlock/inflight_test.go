...
func TestWaitIdleWaitsForEveryActor(t *testing.T) {
	f := NewInFlight()
	releaseA := f.Add("actor-a", "RunWorkload", func() {}, true)
	releaseB := f.Add("actor-b", "CheckpointWorkload", func() {}, false)
...
func TestCancelStartupsCancelsEveryCancelableActor(t *testing.T) {
	...
	f.Add("actor-a", "RunWorkload", func() { canceled["actor-a"] = true }, true)
	f.Add("actor-b", "RestoreWorkload", func() { canceled["actor-b"] = true }, true)
	f.Add("actor-c", "CheckpointWorkload", func() { canceled["actor-c"] = true }, false)
	...
	if canceled["actor-c"] {
		t.Error("a checkpoint was canceled; it must be allowed to finish saving")
	}
}
