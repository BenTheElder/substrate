...
// a set, so an activation writes elements rather than rules.
//
// It did not start that way: the first version emitted two rewrite rules and a
// redirect per actor and rebuilt the whole table on every change. That is
// correct, and it is fine at the handful of actors a worker hosts today, but it
// is O(actors) in the most expensive currency available -- rule expression
// trees, installed under a mutex that serializes every activation on the
// worker. At the thousands per node this design is meant to reach it would have
// become the bottleneck, and it would have arrived as a mysterious slowdown
// rather than a failure.
//
// The table is still emptied of rules and refilled in ONE transaction, for the
// reason that mattered before and still does: committing an empty state
// separately would take addressing away from every actor already running for as
// long as the rebuild takes. AddTable, AddChain, and AddSet are all idempotent
// creates, so a reconcile against an existing table changes only what differs.
//
// The map and set are NOT rebuilt with it. They are diffed against what the
// kernel already holds and only the difference is written, which is what makes
// an activation's cost proportional to the change rather than to the worker's
// occupancy. That began as an optimization and turned out to be a correctness
// requirement -- see reconcileSetElements for the two wire limits a full refill
// runs into, both of which arrive silently.
...
//
// Rewriting the whole set instead would be simpler, and it is what this did
// first. It does not survive contact with a full worker, for two reasons that
// both fail SILENTLY rather than returning an error:
//
//   - The elements of one SetAddElements share a single
//     NFTA_SET_ELEM_LIST_ELEMENTS attribute, whose length is a uint16. This
//     map's elements are 40 bytes each, so 4+40*1639 is the first size past
//     65535: the length wraps, the kernel keeps the fraction the wrapped value
//     describes and logs "65536 bytes leftover after parsing attributes", and
//     the rest of the actors vanish from the map. Measured on a packed worker,
//     the 1639th activation hung with its veth never created.
//   - Splitting across messages clears that, and then the transaction as a
//     whole outgrows what one sendmsg will take (EMSGSIZE, "message too long")
//     somewhere past three of them.
//
// A diff sidesteps both: steady state writes one element, and even a cold
// rebuild is bounded per transaction below. It also removes a window the full
// refill had, where the flush and the repopulate straddled the commit.
...
// applySetElements commits elems ONE netlink message at a time. Both wire
// limits described on reconcileSetElements are then out of reach by
// construction: the message is capped by setElementsPerMessage, and a
// transaction never holds more than one of them.
//
// A cold rebuild therefore lands in several transactions rather than one. That
// is acceptable here and nowhere else in this function: these are additions for
// actors whose veths already exist, so a partial apply leaves some of them
// briefly unwired -- which is the state being recovered from, not a regression
// from it.
...
// message, derived from the encoding rather than guessed, so a change to the
// key or value width cannot quietly walk back over the limit.
//
