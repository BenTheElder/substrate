...
// AllowUnprivilegedPorts lets the current network namespace bind ports below
// 1024 without CAP_NET_BIND_SERVICE. The sysctl is per-namespace, so this
// grants nothing outside the namespace it is called in -- which is how atunnel
// answers a sandbox's DNS on 53 without the worker holding the capability.
...
//
// A name left behind by an earlier incarnation is removed first. The kernel
// creates the name with O_EXCL, so without this a single failed teardown would
// wedge that actor on this worker for good: every later activation of the same
// UID would fail to create the namespace it needs. Removing the name only
// unmounts and unlinks it -- anything still holding the namespace keeps it
// alive -- and an actor is only set up here when the worker is not already
// hosting it, so a name that survives is by definition stale.
