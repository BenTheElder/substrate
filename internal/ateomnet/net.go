...
// DisableReversePathFilter turns off reverse-path validation for the actor
// veths in the current (worker pod) network namespace.
//
// It cannot work here: every actor holds the same frozen address, so every
// host-side veth carries the same /30 and the kernel has one connected route
// per actor for it. Asked which interface 169.254.17.2 is behind, it can only
// answer "the first one". What tells the actors apart is the iifname map in
// nftables, at raw prerouting, before the routing decision.
//
// IP passes because that rewrite lands first. ARP does not: no ip rule sees it,
// arp_process runs the same check on the sender address, and a failure drops
// the request WITHOUT replying -- so an actor that must resolve its own gateway
// cannot.
//
// Only conf.all and conf.default are cleared. The effective setting is the max
// of the two, so eth0 keeps what it was created with and only the veths give it
// up.
...
//
// Without privileged, the runtime bind-mounts /proc/sys read-only. The worker
// holds CAP_SYS_ADMIN and uses no user namespace, so the flag is not locked:
// clear it, write, restore.
