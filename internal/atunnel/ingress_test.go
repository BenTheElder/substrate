...
func TestDeactivateIsolatesLateDials(t *testing.T) {
	...
	if err := server.Activate("team-a", "actor-1", dial); err != nil {
		t.Fatal(err)
	}
	...
	if err := server.Deactivate(context.Background(), "team-a", "actor-1"); err != nil {
		t.Fatal(err)
	}
	receiveWithin(t, firstDone, "first request cancellation")
	if err := server.Activate("team-a", "actor-2", dial); err != nil {
		t.Fatal(err)
	}
	...
	if err := server.Deactivate(context.Background(), "team-a", "actor-2"); err != nil {
		t.Fatal(err)
	}
}
...
func TestServeHTTP(t *testing.T) {
	...
	if err := s.Activate("team-a", "actor-1", testDial); err != nil {
		t.Fatal(err)
	}
...
func TestServeConnectHTTPDialsTheSandbox(t *testing.T) {
	...
	if err := s.Activate("team-a", "actor-1", dial); err != nil {
		t.Fatal(err)
	}
...
func TestIngressRoutesEachActorToItsOwnSandbox(t *testing.T) {
	...
	for _, name := range []string{"actor-1", "actor-2"} {
		if err := s.Activate("team-a", name, testDial); err != nil {
			t.Fatal(err)
		}
