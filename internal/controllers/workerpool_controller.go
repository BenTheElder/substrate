...
func buildDeploymentApplyConfig(wp *atev1alpha1.WorkerPool) *appsv1ac.DeploymentApplyConfiguration {
	...
	microVM := wp.Spec.SandboxClass == atev1alpha1.SandboxClassMicroVM
	...
	// there is no upstream label to select on: ate.dev/sandboxClass=microvm is our
	...
	if microVM {
		podSpec.
			...
			WithNodeSelector(map[string]string{"ate.dev/sandboxClass": string(atev1alpha1.SandboxClassMicroVM)}).
