...
func TestBuildDeploymentPodShape(t *testing.T) {
	tests := []struct {
		...
		{name: "gvisor default", sandboxClass: "", wantKVM: false, wantPlaced: false},
		{name: "gvisor explicit", sandboxClass: atev1alpha1.SandboxClassGvisor, wantKVM: false, wantPlaced: false},
		{name: "microvm", sandboxClass: atev1alpha1.SandboxClassMicroVM, wantKVM: true, wantPlaced: true},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			wp.Spec.SandboxClass = tc.sandboxClass
			...
			// nodeSelector + toleration on ate.dev/sandboxClass.
