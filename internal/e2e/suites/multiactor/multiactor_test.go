...
func TestTwoActorsOnOneWorker(t *testing.T) {
	...
	// Suspending one must not disturb the other. This is the sharp edge of
	// hosting a set: teardown used to be a worker-wide operation -- one interior
	// namespace, one ruleset, one "the actor" to deactivate -- so suspending
	// either actor would have taken the survivor's networking with it. The
	// survivor answering here is what says teardown is now per actor.
