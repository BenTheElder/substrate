...
// (64Mi, held back for cloud-hypervisor + virtiofsd); below a 128Mi guest minimum
// there is no useful headroom. Reject at admission any micro-VM memory limit under
// 192Mi (64Mi reserve + 128Mi guest minimum) so it fails at create time rather than at
// cold boot — a coarse pre-filter; the reserve-aware check in ateom (see
// cmd/ateom-microvm/run.go: resolveGuestMemMiB) stays authoritative. The 192Mi floor
